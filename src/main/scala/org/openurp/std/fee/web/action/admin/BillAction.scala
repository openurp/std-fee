...
class BillAction extends RestfulAction[Bill], ExportSupport[Bill], ImportSupport[Bill], ProjectSupport {
  ...
  def toInitStudentList(): View = {
    val semester = entityDao.get(classOf[Semester], getIntId("bill.semester"))
    ...
    students.foreach { student =>
      ...
      configHelper.find(currentState) match {
        case Some(config) =>
          val sum = sumFeeMap.getOrElse(student.id, 0L)
  ...
  def downloadTemplate(): Any = {
    ...
    schema.generate(os)
    Stream(new ByteArrayInputStream(os.toByteArray), MediaTypes.ApplicationXlsx, "收费信息.xlsx")
  }
