...
[@b.head/]
  ...
  [@b.form name="tuitionConfigForm" theme="list" action=b.rest.save(tuitionConfig) ]
    ...
    [@b.select label="起始年级" name="tuitionConfig.fromGrade.id" value=(tuitionConfig.fromGrade)! required="true" items=grades style=elementSTYLE/]
    [@b.select label="截止年级" name="tuitionConfig.toGrade.id" value=(tuitionConfig.toGrade)! items=grades style=elementSTYLE/]
    ...
    [@b.textfield label="学制" name="tuitionConfig.duration" value=(tuitionConfig.duration)! required="true"  maxlength="3"/]
    ...
    [@b.select label="专业" name="tuitionConfig.major.id" items=majors?sort_by(["name"]) empty="..." value=(tuitionConfig.major.id)! required="false" option=r"${item.code} ${item.name}" style=elementSTYLE /]
    [@b.select label="专业方向" name="tuitionConfig.direction.id" items=directions?sort_by(["name"]) empty="..." value=(tuitionConfig.direction.id)! option=r"${item.code} ${item.name}" required="false" style=elementSTYLE /]
