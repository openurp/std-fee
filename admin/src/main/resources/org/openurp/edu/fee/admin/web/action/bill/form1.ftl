...
[@b.head/]
  ...
  [@b.form name="billForm" action="!save" target="bills" theme="list"]
    ...
    [#if (bill.id)?exists]
      ...
      [@b.field label="应缴费用"]<span style="display: inline-block;" >${bill.amount?string("0.00#")}</span>分[/@]
      [@b.textfield label="本次缴费" name="bill.payed" value=(bill.payed?string("0.00#"))! required="true" maxlength="64" check="match('number')" style=elementSTYLE  comment="分"/]
    [#else]
      ...
      [@b.textfield label="应缴费用" name="bill.amount" value=(bill.amount?string("0.00#"))! required="true" maxlength="64" check="match('number')" style=elementSTYLE comment="分"/]
      [@b.textfield label="本次缴费" name="bill.payed" value=(bill.payed?string("0.00#"))! maxlength="64" check="match('number')" style=elementSTYLE  comment="分"/]
    [/#if]
    [#if (bill.id)?exists]
